...
INSERT_CLEAR = -0.3  # mm — négatif = serrage (press-fit) : insert 0.2mm plus large que le creux
LIP_W        = 1.5   # mm — largeur de la collerette (débord sur la surface océan)
...
def build_continent_inserts(land_map: np.ndarray, red_mask: np.ndarray = None):
    ...
    """
    from scipy.ndimage import label as ndlabel, binary_erosion, binary_dilation, binary_closing, binary_opening
    ...
    # 2) Ré-appliquer les coupures rouges dans lq (survivent au binary_closing)
    if red_mask is not None:
    ...
    for cid in range(1, n_comp + 1):
        ...
        # mask_lip  = contour exact du continent (lissé) → ce qu'on voit d'en haut
        # mask_body = continent érodé de n_lip quads    → s'insère dans le trou du globe
        # La jupe (= bord du continent) repose sur le rebord du trou → rétention assurée.
        from scipy.ndimage import gaussian_filter as _gf
        _sigma_lip = max(n_lip * 1.5, 4.0)
        # threshold 0.45 ≈ frontière originale lissée ; | mask garantit la couverture intérieure
        mask_lip = (_gf(mask.astype(np.float32), sigma=_sigma_lip) > 0.45) | mask

        # Corps : érodé de n_lip → plus petit que le trou, glisse dedans sans forcer
        mask_body = binary_erosion(mask, iterations=n_lip)
        if not mask_body.any():   # fallback pour très petites îles
            mask_body = mask
        ...
        print(f"    {name:20s}: {mask.sum():>8,} quads  (collerette {LIP_W}mm)")
