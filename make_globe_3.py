...
# ── Configuration ─────────────────────────────────────────────────────────────
SVG_FILE   = Path.home() / "Desktop/globe teresstre/BlankMap_World_simple3.svg"
...
SNAP_H       = 6.0   # mm — hauteur/profondeur du tenon
SNAP_CLEAR   = 0.3   # mm — jeu d'assemblage (mortaise = SNAP_R + SNAP_CLEAR)
...
def _cylinder_walls(r: float, z0: float, z1: float, outward: bool) -> np.ndarray:
    ...
    rb = _snap_pts(r, z0);  rt = _snap_pts(r, z1)
...
def build_globe_north(land_map: np.ndarray) -> np.ndarray:
    ...
    walls = _cylinder_walls(SNAP_R, z0=0.0, z1=-SNAP_H, outward=True)
    bot   = _disk(SNAP_R, z=-SNAP_H, normal_pos_z=False)
...
def build_globe_south(land_map: np.ndarray) -> np.ndarray:
    ...
    walls  = _cylinder_walls(r_sock, z0=0.0, z1=-SNAP_H, outward=False)
    bot    = _disk(r_sock, z=-SNAP_H, normal_pos_z=True)
    return np.concatenate([dome, cap, walls, bot]).astype(np.float32)
...
def build_continent_inserts(land_map: np.ndarray):
    ...
    px_mm = 2 * np.pi * RADIUS / land_map.shape[1]   # taille d'1 pixel à l'équateur (mm)
    ...
    # closing arrondit les coins rentrants, opening arrondit les coins saillants
    # → réduit les dents de scie sur les murs latéraux des inserts.
